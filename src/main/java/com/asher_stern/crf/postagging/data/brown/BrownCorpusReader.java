package com.asher_stern.crf.postagging.data.brown;
...
public class BrownCorpusReader implements Iterator<List<TaggedToken<String, String>>>
{
	...
	private void initFiles()
	{
		...
		if (!directory.exists()) {throw new CrfException("Directory "+directory.getAbsolutePath()+" does not exist.");}
		if (!directory.isDirectory()) {throw new CrfException("Directory "+directory.getAbsolutePath()+" is not a directory.");}
		...
		{
			throw new CrfException("Failed to read catalog file of Brown corpus.",e);
		}
	}
	...
	private void createSentenceIterator(File file)
	{
		...
		{
			throw new CrfException("Failed to read file: "+file.getAbsolutePath(),e);
		}
	}
