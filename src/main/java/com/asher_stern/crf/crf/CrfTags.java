...
public class CrfTags<G> implements Serializable
{
	...
	{
		if (!canFollow.keySet().containsAll(tags)) {throw new CrfException("map keys do not contain all tags");}
		if (!canPrecede.keySet().containsAll(tags)) {throw new CrfException("map keys do not contain all tags");}
		if (!canPrecedeNonNull.keySet().containsAll(tags)) {throw new CrfException("map keys do not contain all tags");}
		if (!precedeWhenFirst.keySet().containsAll(tags)) {throw new CrfException("map keys do not contain all tags");}

		if (tags.contains(null)) {throw new CrfException("tags (the set of tags) should not contain null.");}
		
		for (G tag : canPrecedeNonNull.keySet())
		{
			if (canPrecedeNonNull.get(tag).contains(null)) {throw new CrfException("BUG");}
		}
		...
		{
			throw new CrfException("Error: canFollow does not contain null key. This means that it is not specified which tags can be assigned to a first token of a sentence.");
		}
	}
	...
	private void consistencyCheck()
	{
		for (G tag : canFollow.keySet())
		{
			for (G follow : canFollow.get(tag))
			{
				...
				{
					throw new CrfException("can-follow and can-preced are inconsistent.");
				}
			}
		}
